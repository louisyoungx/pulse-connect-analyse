    ...
    <div class="setting-main">
        ...
        <van-cell-group class="setting-card">
            <van-cell title="显示特征值" label="显示脉搏特征点">
                <template #right-icon>
                    <van-switch v-model="settings.choice.eigenvalueOpen" @click="eigenvalueOpen()" size="24" />
                </template>
            </van-cell>
            <van-cell title="启动动画" label="可能会造成卡顿">
                <template #right-icon>
                    <van-switch v-model="settings.choice.animationOpen" @click="animationOpen()" size="24" />
                </template>
            </van-cell>
        </van-cell-group>
        ...
        <van-cell-group class="setting-card">
            <van-form @submit="saveSettings()">
                <div style="margin: 16px;">
                    <van-button @click="saveSettings()" round block type="info" native-type="submit">保存</van-button>
                </div>
            </van-form>
        </van-cell-group>
        <br>
    ...
    export default {
        ...
        methods:{
            ...
            updateFPS() {
                ...
                const flash = this.settings.Flash;
                let FPS;
                FPS = (speed / flash) * 1000
